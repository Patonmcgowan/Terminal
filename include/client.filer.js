/**
 * client.filer.js (refactored from \js\filer.js)  https://github.com/ebidel/idb.filesystem.js
 *
...
/*
  ...
  11 Feb 2022 MDS Renamed from filer.js to client.filer.js
                  Minor mods to allow to work on a standalone browser client
                  (refer lines commented with "MDS")
  22 Feb 2022 MDS bug fixes to createDir
...
var Filer = new function() {
  ...
  var isFsURL_ = function(path) {
    ...
    // return path.indexOf(FS_URL_SCHEME) == 0; 220209 MDS
    return path.indexOf(FILE_BASE) == 0;
  };

  // Path can be relative or absolute. If relative, it's taken from the cwd_.
  // If a filesystem URL is passed to it, it is simply returned
  var pathToFsURL_ = function(path) {
    if (!isFsURL_(path)) {
      if (path[0] == '/') {
        ...
        // Modified 220209 MDS because root directory entries weren't getting created correctly - extra slash was being inserted
        // I'm not sure if this is an issue on *NIX machines so only do it for Windows boxes for now
  ...
  Filer.prototype.init = function(opt_initObj, opt_successCallback,
    ...
    var name = initObj.name || 'temporary';
  ...
  Filer.prototype.mkdir = function(path, opt_exclusive, opt_successCallback,
    ...
    var createDir = function(rootDir, folders) {
      ...
      while (folders[0] == '.' || folders[0] == '') { // 220223 MDS Changed from 'if' to 'while'
        folders = folders.slice(1);
      }
      if (folders.length == 0) // 220223 Added MDS in case the end element of /foo/bar/ is the only one
        return;
