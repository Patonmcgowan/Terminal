/**
 * client.idb.filesystem.js (refactored from \idb.filesystem.js-master\src\idb.filesystem.js)  
...
/*
  ...
  Revision History
  ================
  02 Feb 2022 MDS Downloaded from Eric Bidelman https://github.com/ebidel/idb.filesystem.js
  11 Feb 2022 MDS Renamed from idb.filesystem.js to client.idb.filesystem.js
                  Minor mods to allow to work on a standalone browser client
                  (refer lines commented with "MDS")
  22 Feb 2022 MDS Changed indexeddb definition at top of file
                  Made FILE_STORE_ the consistently used identifier through the file
                  Create objectStore if it doesn't exist in the database
...
// Disabled the following because self.webkitRequestFileSystem doesn't work when running from File:\\  
//    220208 MDS
...
// 220222 MDS const indexedDB = exports.indexedDB || exports.mozIndexedDB || exports.msIndexedDB;
...
Entry.prototype = {
  ...
  toURL: function() {
    ...
    //        this.fullPath;
    // let base = origin + DIR_SEPARATOR + FILE_STORE_.toLowerCase();
    if (this.fullPath.substr(0,1) == "/") 
      return FILE_BASE + this.fullPath.substr(1);
    return FILE_BASE + this.fullPath;
  },
};
...
function resolveLocalFileSystemURL(url, successCallback, opt_errorCallback) {
  ...
  url = url.replace(FILE_BASE, '/'); // Replace with a "/" because every entry starts with this
  if (url.substr(-1) === '/') {
    url = url.slice(0, -1);
  }
  if (url) {
...
// ----------------------------------------------------------------------------
// MDS 220208 New function to replace original idb_.open
self.onError = function(e) {
  console.log(e);
}

idb_.open = function(dbName, successCallback, opt_errorCallback) {
  typeof dbName == 'undefined' ? idb_.dbName = 'fileSystem' :idb_.dbName = dbName;

  let indexedDB = window.indexedDB || window.webkitIndexedDB || window.mozIndexedDB || window.msIndexedDB;
  idb_.db = indexedDB.open(idb_.dbName);

  idb_.db.onsuccess = function (e) {
    idb_.db = e.target.result;
    idb_.db.onerror = function(e){
      console.log(e);
    };

    if (!idb_.db.objectStoreNames.contains(FILE_STORE_)) {
      // We need to write a new object store, so close the database and reopen it with a higher 
      // version to trigger the onupgradeneeded event (which is the only place in which the
      // object store can be created) 
      let version =  parseInt(idb_.db.version);
      idb_.db.close();
      idb_.db = indexedDB.open(idb_.dbName, version+1);

      // This will always happen when we open the second request
      idb_.db.onupgradeneeded = function (e) { 
        idb_.db = e.target.result;
        idb_.db.createObjectStore(FILE_STORE_ /*,{keyPath: 'id', autoIncrement: true}*/ );
      };
  ...
  idb_.db.onerror = opt_errorCallback || onError;
  idb_.onblocked = opt_errorCallback || onError;

  idb_.db.onupgradeneeded = function (e) {
    idb_.db = e.target.result;
    idb_.db.onerror = onError;
    ...
    if (typeof successFn !== 'undefined') 
      successFn('IndexedDB file system `' + idb_.dbName + '\\' + FILE_STORE_ + '` created');
  };
}
...
window.testIt = function(f) { // 220208 MDS
  var tx = idb_.db.transaction([FILE_STORE_], 'readonly');
  var range = IDBKeyRange.bound(f, f + DIR_OPEN_BOUND,
                                false, true);
...
window.testAll = function() { // 220208 MDS
  var tx = idb_.db.transaction([FILE_STORE_], 'readonly');
  var request = tx.objectStore(FILE_STORE_).getAll();
  tx.onabort = onError;
  tx.oncomplete = function(e) {
    console.log(request.result);
  };
...
// 220209 MDS Consolidated FILE_BASE definitions throughout code
let FILE_BASE;
if (navigator.userAgentData.platform.indexOf("Win")  != -1) {
  // Do something different for Windows because of the way it handles directory separators.
  // I'm not sure if this is an issue with *NIX systems so we leave those as is 
  // (code untested on *NIX, Android and MacOS)
  if (location.host.length == 0) {
    FILE_BASE = location.protocol + '//';
  } else {
    FILE_BASE = location.protocol + '//' + location.host + DIR_SEPARATOR;
  }
} else { // All non Windoze systems
  FILE_BASE = location.protocol + '//' + location.host + DIR_SEPARATOR;
}
...
if (exports === window && exports.RUNNING_TESTS) {
  ...
  // 220209 MDS TODO Create & export FILE_BASE and modify filer.js accordingly - put FILE_STORE & DIR_SEPARATOR back inside this function
}
